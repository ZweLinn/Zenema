        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 text-center m-auto max-w-7xl mt-10">
          {
            creditsMovieData.map((movie) => (
                <div key={movie.id + Math.floor(Math.random() * 1000)} className="rounded-lg flex flex-col items-center justify-center h-fit cursor-pointer group" onClick={() => handleClick(movie.id)}>
